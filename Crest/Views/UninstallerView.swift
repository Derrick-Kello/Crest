...
struct UninstallerView: View {
    ...
    private var filteredApps: [CatalogItem] {
        let trimmed = appQuery.trimmingCharacters(in: .whitespaces)
        let apps = viewModel.commandBar.apps
        guard !trimmed.isEmpty else { return apps }

        // Same matcher the command bar uses, so an app found by typing "vscode"
        // there is found by typing "vscode" here.
        let tokens = FuzzyMatch.tokenize(trimmed)
        let joined = tokens.joined()
        return apps
            .compactMap { app -> (CatalogItem, Int)? in
                guard let score = FuzzyMatch.score(tokens: tokens, joined: joined, keys: app.keys) else {
                    return nil
                }
                return (app, score)
            }
    ...
    private func appRow(_ app: CatalogItem) -> some View {
        let path = app.filePath ?? ""
        return Button {
            Task { await viewModel.chooseAppToUninstall(at: URL(fileURLWithPath: path)) }
        } label: {
            HStack(spacing: 10) {
